...
public class TranslatorActivity extends BaseActivity {
    ...
    private void createSpinner() {
        if (this.isEngLanguage()) {
            this.spSourceAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_dropdown_item, TranslatorActivity.spSourceList_en);
            this.spDestAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_dropdown_item, TranslatorActivity.spDestList_en);
        } else {
            this.spSourceAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_dropdown_item, TranslatorActivity.spSourceList);
            this.spDestAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_dropdown_item, TranslatorActivity.spDestList);
        }
    ...
    private void updateDestLanguage(String code) {
        if (code.equalsIgnoreCase(TranslatorActivity.SOURCE_LANGUAGE_CODE[0]) || code.equalsIgnoreCase(TranslatorActivity.spSourceList.get(0))) {
    ...
    private void updateOutputText(final String text) {
        ...
        this.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                TranslatorActivity.this.tvOutputString.setText(String.format(Locale.ENGLISH, text));
    ...
    private void updateInputText(final String text) {
        ...
        this.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                TranslatorActivity.this.etInputString.setText(String.format(Locale.ENGLISH, text));
    ...
    private String getEnLanguageName(String code) {
        int index = 0;
        for (int i = 0; i < TranslatorActivity.codeList.size(); i++) {
            if (code.equalsIgnoreCase(TranslatorActivity.codeList.get(i))) {
                index = i;
                return TranslatorActivity.languageList.get(index);
            }
        }
        return code;
