...
public class StillImageSegmentationTransactor extends BaseTransactor<MLImageSegmentation> {
     ...
     */
    public StillImageSegmentationTransactor(Context context, MLImageSegmentationSetting options, Bitmap originBitmap, ImageView imageView, int detectCategory) {
     ...
     * @param detectCategory   -1 represents all detections, others represent the type of replacement color currently detected.
     */
    public StillImageSegmentationTransactor(Context context, MLImageSegmentationSetting options, Bitmap originBitmap, Bitmap backgroundBitmap, ImageView imageView, int detectCategory) {
        this.context = context;
        this.detector = MLAnalyzerFactory.getInstance().getImageSegmentationAnalyzer(options);
        this.originBitmap = originBitmap;
    ...
    protected void onSuccess(
        ...
        int[] pixels;
        if (results == null) {
            Log.i(StillImageSegmentationTransactor.TAG, "detection failed");
            return;
        }
        if (results.getMasks() == null) {
            Log.i(StillImageSegmentationTransactor.TAG, "detection failed, none mask return");
            return;
        }
